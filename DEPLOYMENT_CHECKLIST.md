...
# Deployment Checklist (v1.04.6 / Ollama)
...
- [x] Cloud batch prescription + `--no-cloud-prescription` for heavy `:cloud` models
...
```bash
git tag -a v1.04.6 -m "Release v1.04.6"
git push origin v1.04.6
```

After assets publish:

1. Verify release archives + `.sha256` files
2. Update Homebrew tap formula version + SHA256 (`./update-homebrew-tap.sh 1.04.6`)
