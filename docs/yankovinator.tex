...
Yankovinator consists of the following main components:

\begin{enumerate}
    \item \textbf{SyllableCounter}: Analyzes syllable structure using NaturalLanguage framework
    \item \textbf{RhymeSchemeAnalyzer}: Detects and groups rhyming lines from original lyrics
    \item \textbf{OEDDictionary}: Suggests dictionary-backed word substitutions
    ...
    \item \textbf{BenchmarkRunner}: Measures generation performance for the \texttt{benchmark} CLI
...
\begin{itemize}
    \item \textbf{Swift 5.10+}: Primary programming language (Swift Package Manager)
    \item \textbf{NaturalLanguage}: Apple framework for linguistic analysis
    \item \textbf{Ollama}: Local LLM for text generation (\texttt{llama3.2:3b} default)
...
\begin{lstlisting}
yankovinator --input-dir <songs-dir> --themes-dir <themes-dir> \\
            --output-dir <out-dir> [--workers <n>] [--candidates <n>] \\
            [--ollama-url <url>] [--model <model-name>] \\
            [--analyze] [--verbose] [--force]
\end{lstlisting}

Batch output layout: \texttt{<output-dir>/<theme>/<song>.parody.txt}. Each theme file is a \texttt{keyword: definition} list (same format as before).
...
\begin{itemize}
    ...
    \item \texttt{ollamaModel: String} - Ollama model name (default: llama3.2:3b)
\end{itemize}
...
Run tests with (requires full Xcode for XCTest):
\begin{lstlisting}[language=bash]
export DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer
swift test
\end{lstlisting}

Sample inputs for local runs:
\begin{lstlisting}[language=bash]
...
swift run benchmark --lyrics data/example_lyrics.txt \\
  --keywords data/example_keywords.txt --iterations 5
\end{lstlisting}
...
\begin{itemize}
    ...
    \item Full Xcode app when running \texttt{swift test}
\end{itemize}
