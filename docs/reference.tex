...
\begin{itemize}
    ...
    \item \texttt{BenchmarkRunner} - Performance measurement for the \texttt{benchmark} CLI
\end{itemize}
...
\section{OEDDictionary}

Optional dictionary helper that suggests syllable-matched words using the 1913 Oxford / Webster dictionary (Project Gutenberg text, cached locally when downloaded).
...
\textbf{Notes:}
\begin{itemize}
    \item If \texttt{dictionaryPath} exists, the file is loaded immediately
    \item Otherwise the dictionary may be downloaded asynchronously and cached
    \item Dictionary support is optional; generation still works without it
\end{itemize}

\subsection{Usage in generation}

\texttt{ParodyGenerator} constructs an \texttt{OEDDictionary} when \texttt{useDictionary} is \texttt{true} (default) and feeds position-aware suggestions into Ollama prompts.
...
\begin{itemize}
    ...
    \item \textbf{v1.06.10} - Per-worker progress bars show elapsed + remaining (line-based ETA when available)
    \item \textbf{v1.06.9} - Ncurses-like \texttt{yankovinator-tui}: color boxes + emoji progress bars per worker thread
    \item \textbf{v1.06.8} - Expanded certification battery (unit, regression, UX, A/B, blackbox, fit-optimize E2E); docs + Pages report
    \item \textbf{v1.06.7} - Fix parallel \texttt{--fit-optimize} NaturalLanguage segfault (shared embeddings + safe fit scoring)
    \item \textbf{v1.06.6} - PATH-resolve \texttt{yankovinator-tui}; ban Swift multiline worker TUI (segfault); MIDI only with Rust TUI
    \item \textbf{v1.06.5} - Multi-worker segfault fix (MIDI prewarm, no alt-screen worker pool); \texttt{--version}; local install script
    \item \textbf{v1.06.4} - Batch resume checkpoint (\texttt{.yankovinator/}, \texttt{--fresh-batch}); Rust \texttt{yankovinator-tui} dashboard
    \item \textbf{v1.06.0} - Batch-only \texttt{yankovinator} CLI (\texttt{--input-dir}, \texttt{--themes-dir}, \texttt{--output-dir}); removed single-file lyrics arg and \texttt{--keywords}
    \item \textbf{v1.05.0} - Up to 128 parallel workers; multi-candidate batch ranking; cloud batch prescription
    \item \textbf{v1.01} - Ollama \texttt{llama3.2:3b}, OED suggestions, \texttt{benchmark} CLI, sample data under \texttt{data/}
\end{itemize}
